...
- **ADD**: `WBSTemplateItem` entity (template-tree node) — `{id, template_id FK, parent_id FK NULL, code VARCHAR(32) (e.g., "2.4"), name_en, name_ro, default_unit VARCHAR(32) NULL, sort_order INT, depth INT}`. Hierarchical via `parent_id`.
- **ADD**: `WBSItem` entity (per-Project clone, fully editable) — `{id, project_id FK, parent_id FK NULL, code, name, default_unit, sort_order, depth, source_template_item_id FK NULL (traceability — which template node this was cloned from; NULL for user-created), notes}`. NOT bilingual — owns one user-chosen label.
